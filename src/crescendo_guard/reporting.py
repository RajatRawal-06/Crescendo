...
def write_technical_report(results: dict[str, object], output_dir: str | Path) -> Path:
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    report_path = output_path / "technical_report.md"
...
def _render_report(results: dict[str, object]) -> str:
    ...
    layered = next(row for row in rows if row["strategy"] == "layered")
    for run in layered["attack_runs"]:
        turn = run["first_intercept_turn"] or "none"
        final_risk = run["turns"][-1]["risk"]["cumulative_risk"]
        detail_lines.append(
            f"- `{run['id']}` ({run['category']}): first intercept turn={turn}, final cumulative risk={final_risk:.2f}."
        )
    attack_detail = "\n".join(detail_lines)

    return f"""# CrescendoGuard: An Elite Defense Pipeline for Escalating Conversational Jailbreaks
